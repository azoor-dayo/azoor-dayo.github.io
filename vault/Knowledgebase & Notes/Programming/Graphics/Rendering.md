...
### Faceted Shading
Simple per-triangle shading is computable by:
For each triangle face, compute the normal of this triangle. You can cross product 2 vectors created by the 3 pts of this triangle.
Pass this normal value + light vector into each pixel shader, and compute the color with these 2 vectors. Done.

End result will look very... triangl-y. Each triangle face will be visibly shaded, but it's super fast. 
[insert image here]

### Phong Shading
Taking the 3 vertex normals of a triangle (obtained from model file), interpolate them (barycentric interpolation or whatever)  to find the interpolated normal for each pixel. Fragment shader shld be able to do this fast.

Then apply phong lighting to the pixel with the normal, bam done.
...
The normal map shown above is done in object space. That's fine for objects that don't 

Normal maps are good on static objects, but when the object starts to rotate, the normals will still remain pointing in the same world-space orientation as before, even when the object itself has rotated. The normals of the object should be updated along with the object itself. What can we do to rememdy this?

The solution is tangent space.
![](https://upload.wikimedia.org/wikipedia/commons/thumb/6/66/Image_Tangent-plane.svg/1920px-Image_Tangent-plane.svg.png)

So what exactly is tangent space, and what's the difference?

Tangent Space: 
Instead of storing normals in world space, we store the normals in tangent space. Doing this will require the world space normals to be recomputed from the tangent space normals in real time, which has some overhead. But, you'd be able to freely rotate the object around and get the correct normals, as each normal from the tangent space normal map is tied to the object's orientation, and not world space. Cool!
...
https://help.autodesk.com/view/MAYAUL/2022/ENU/?guid=GUID-0CB26A3B-DB26-42E1-AAAB-4314B637C341 
![](https://i0.wp.com/allegrodigital.com/uploaded_images/normals-797421.jpg?ssl=1)

### World, object and tangent space normals
https://blenderartists.org/t/tangent-vs-object-space-vs-world-space/456423
https://polycount.com/discussion/227725/difference-between-object-space-and-world-space-normals
